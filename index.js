...
const NETWORKS = {
    BASE: {
      ...
      EXPLORER_URL: "https://evm.hyperstats.xyz",
    ...
    async handleGrowGardenMode() {
      const { autoRepeat } = await inquirer.prompt([
        {
          ...
          name: "autoRepeat",
          message: this.colors.style(
            "Auto repeat process every 30-35 minutes?",
            "menuTitle"
          ),
          default: true,
        },
      ...
      if (autoRepeat) {
        while (true) {
          ...
          const minutes = Math.floor(Math.random() * (30 - 35 + 1)) + 30; // Waktu acak antara 60 hingga 70 menit
          const totalSeconds = minutes * 60; // Mengonversi menit ke detik
